...
import { updateMainCategory } from "../../mainCategory.service";
import { getCategories } from "../../../category/category.service";
import { addMainCategoryValidationSchema } from "../../mainCategory.utils";
import { MainCategoryDataType, EditMainCategoryModalProps } from "../../mainCategory.types";
...
const EditMainCategoryModal: FC<EditMainCategoryModalProps> = ({ isOpen, onClose, initialValues, reloadData }) => {
    ...
    const handleSubmit = async (
        ...
        try {
            ...
            const response = await updateMainCategory(formData);
            ...
            toast.error("An error occurred while updating the main category.");
            console.error(error);
        } finally {
            setSubmitting(false);
            setIsLoading(false);
        ...
        <div onClick={onClose} className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center cursor-pointer">
            <div onClick={(e) => e.stopPropagation()} className="bg-white rounded-lg w-full max-w-xl max-h-[90vh] overflow-hidden flex flex-col mx-3">
                <div className="flex justify-between items-center p-4 border-b">
                    <h2 className="font-semibold text-lg">Edit Main Category</h2>
