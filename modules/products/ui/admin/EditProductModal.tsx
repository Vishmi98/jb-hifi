...
import { AddProductFormValues, EditProductModalProps, PaymentMethodDataType, SellTypeDataType, TagLineDataType } from "../../products.types";
import { createProduct, getPaymentMethods, getSellTypes, getTagLines, updateProduct } from "../../products.service";
...
const EditProductModal: React.FC<EditProductModalProps> = ({ isOpen, onClose, initialValues, reloadData }) => {
    // Media States
    ...
    useEffect(() => {
        ...
        const loadInitialData = async () => {
            ...
            try {
                // Fetch root-level reference options
                ...
                // Cascading pre-fetch for existing product category selections
                if (initialValues.categoryId) {
                    const mainRes = await getMainCategoryByCategory({ categoryId: initialValues.categoryId });
                    if (mainRes?.success) setMainCategories(mainRes.mainCategories || []);
                }

                if (initialValues.mainCategoryId) {
                    const subRes = await getSubCategoryByMainCategory({ mainCategoryId: initialValues.mainCategoryId });
                    if (subRes?.success) setSubCategories(subRes.subCategories || []);
                }

                if (initialValues.subCategoryId) {
                    const leafRes = await getLeafCategoryBySubCategory({ subCategoryId: initialValues.subCategoryId });
