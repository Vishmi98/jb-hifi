...
interface LeafCategoryCardProps {
    category: LeafCategoryDataType;
    href?: string;
}

export const LeafCategoryCard: React.FC<LeafCategoryCardProps> = ({ category, href }) => {
    // Default fallback if href isn't provided directly
    const targetHref = href || `/collections/${category.categoryInfo?.slug}/${category.mainCategoryInfo?.mainSlug}/${category.subCategoryInfo?.subSlug}/${category.leafSlug}`;
