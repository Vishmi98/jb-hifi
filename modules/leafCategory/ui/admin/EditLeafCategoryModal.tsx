...
import { updateLeafCategory } from "../../leafCategory.service";
...
import { addLeafCategoryValidationSchema } from "../../leafCategory.utils";
import { LeafCategoryDataType, EditLeafCategoryModalProps } from "../../leafCategory.types";
...
const EditLeafCategoryModal: FC<EditLeafCategoryModalProps> = ({ isOpen, onClose, initialValues, reloadData }) => {
    ...
    const handleSubmit = async (
        ...
        try {
            ...
            const response = await updateLeafCategory(formData);
            ...
            toast.error("An error occurred while updating the leaf category.");
            console.error(error);
        } finally {
            setSubmitting(false);
            setIsLoading(false);
        ...
        <div onClick={onClose} className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center cursor-pointer">
            <div onClick={(e) => e.stopPropagation()} className="bg-white rounded-lg w-full max-w-xl max-h-[90vh] overflow-hidden flex flex-col mx-3">
                <div className="flex justify-between items-center p-4 border-b">
                    <h2 className="font-semibold text-lg">Edit Leaf Category</h2>
...
export default EditLeafCategoryModal;
