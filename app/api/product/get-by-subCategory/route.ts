...
export async function POST(req: NextRequest) {
    try {
        ...
        // 2. Query MongoDB
        // Note: Use .find() if a subCategoryId can belong to multiple products, or .findOne() if it's strictly a 1:1 relation.
        ...
        if (!products || products.length === 0) {
            return sendErrorResponse("Products not found for this subCategoryId", 200);
        }
        ...
        return sendErrorResponse(
            error?.message || "Unexpected error occurred while fetching products by subCategoryId",
            200
        );
    }
}